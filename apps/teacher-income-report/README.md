...
页面的“下载模板”按钮可以直接生成正确表头。课时明细支持修改课程单价和数值乘数，
乘数默认自动读取老师当月课时系数（例如 62% 对应 `0.62`）。老师实际课时费为
`调整后总金额 × 乘数`。课时明细下方可添加“额外收入与扣款”，其中正数为额外收入、负数为罚款；
课时奖金小计与这部分合计后形成课时反馈奖金。课程单价调整会同步改变学生归属和服务提成的计提基数，乘数不会改变该基数。
页面和下载的 Excel 都会分别展示额外项目明细、合计和最终课时反馈奖金。
