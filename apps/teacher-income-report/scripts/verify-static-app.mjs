...
assert.match(html, /teacherScoresInput/, "must upload monthly teacher scores CSV");
...
assert.match(app, /data-lesson-unit-price/, "lesson fee details must support editable unit prices");
assert.doesNotMatch(app, /data-lesson-discount/, "lesson fee details must not expose discounts");
assert.match(app, /data-lesson-multiplier/, "lesson fee details must support numeric multipliers");
assert.match(app, /data-add-extra/, "income details must support adding extra income and penalties");
assert.match(app, /调整金额（元）/, "extra adjustments must use a clear amount label");
...
assert.match(css, /\.extra-adjustment-row/, "extra adjustment editor must be styled");
...
assert.match(css, /\.score-top50/, "top-half monthly scores must be highlighted");
assert.match(css, /\.ledger-row\.warning/, "missing monthly score reason must be highlighted");
