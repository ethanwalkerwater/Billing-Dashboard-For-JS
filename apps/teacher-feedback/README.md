...
- 老师在某项指标至少有 1 个有效评分时，对于“当月有上课但该学员没有给该老师该项评分”的情况，会用**当月该项平均分**补齐后再参与老师评分，避免因样本过少导致老师分数虚高。
- 老师在某项指标的有效评分数为 0 时，**不会使用机构月均分生成老师分数**：汇总数值留空，精简评分表显示“无评分”，且不参与总评分和排名。
...
- `outputs/teacher-feedback/2026-02/teacher_scores.csv`：精简老师评分；各项保留三位小数，无有效评分时明确显示“无评分”
